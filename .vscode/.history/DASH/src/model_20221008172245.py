...
#         # no of examples at each step/batch
        ...
        if config_kwargs['arch_retrain_default'] is not None:
            ...
            arch_retrain = arch
        self.arch_retrain = arch_retrain
        if args.baseline:
    ...
    def train(self, dataset, val_dataset=None, val_metadata=None, remaining_time_budget=None):
        ...
        ****************************************************************************
        ...
        args = self.args
        arch = 'convnext'
        lr_sched_iter = 'convnext'
        dims, sample_shape, num_classes, batch_size, epochs, loss, lr, arch_lr, weight_decay, opt, arch_opt, weight_sched_search, weight_sched_train, accum, clip, retrain_clip, validation_freq, retrain_freq, \
        einsum, retrain_epochs, arch_default, kernel_choices_default, dilation_choices_default, quick_search, quick_retrain, config_kwargs = get_config(self.task)  

        # TODO: args.dataset is not the same dataset as is given
        ...
        for ep in range(epochs):
            train_loss = train_one_epoch(self.model, self.optimizer, self.scheduler, self.args.device, train_loader, loss, clip, 1, self.n_train_temp, decoder,
            ...
            if args.verbose and not args.baseline and (ep + 1) % args.print_freq == 0:
                print_grad(self.model, args.kernel_choices, args.dilation_choices)

            if ep % validation_freq == 0 or ep == epochs - 1: 
                if args.baseline:
                    val_loss, val_score = evaluate(self.model, args.device, val_loader, loss, metric, n_val, decoder, transform, fsd_epoch=ep if args.dataset == 'FSD' else None)
                    train_score.append(val_score)

                    if (ep + 1) % args.print_freq == 0  or ep == epochs - 1: 
                        print("[train", ep, "%.6f" % self.optimizer.param_groups[0]['lr'], "] time elapsed:", "%.4f" % (default_timer() - time_start), "\ttrain loss:", "%.4f" % train_loss, "\tval loss:", "%.4f" % val_loss, "\tval score:", "%.4f" % val_score, "\tbest val score:", "%.4f" % compare_metrics(train_score))

                    np.save(os.path.join(args.save_dir, 'train_score.npy'), train_score)

                elif (ep + 1) % args.print_freq == 0 or ep == epochs - 1:
                    print("[train", ep, "%.6f" % self.optimizer.param_groups[0]['lr'], "] time elapsed:", "%.4f" % (default_timer() - time_start), "\ttrain loss:", "%.4f" % train_loss)
            ...
            if not args.baseline and ((ep + 1) % retrain_freq == 0 or ep == epochs - 1):
                ...
                for name, param in model.named_arch_params():
                    ...
                    ks.append(args.kernel_choices[int(param_values[-1] // len(args.dilation_choices))])
                    ds.append(args.dilation_choices[int(param_values[-1] % len(args.dilation_choices))])
                ...
                if prev_param_values is not None and torch.equal(param_values, prev_param_values):
                    ...
                    if os.path.isfile(os.path.join(args.save_dir, 'network_hps.npy')):
                        ...
                        search_scores = []
                        search_train_loader, search_val_loader, search_test_loader, search_n_train, search_n_val, search_n_test, search_data_kwargs = get_data(args.dataset, accum * batch_size, self.arch_retrain, True)
                        retrain_model = get_model(self.arch_retrain, sample_shape, num_classes, config_kwargs, ks = ks, ds = ds)
                        ...
                        for lr, drop_rate, weight_decay, momentum in hp_configs:
                            ...
                            retrain_model = get_model(self.arch_retrain, sample_shape, num_classes, config_kwargs, ks = ks, ds = ds, dropout = drop_rate)
                            retrain_model = MixtureSupernet.create(retrain_model.cpu(), in_place=True)
                            retrain_model = retrain_model.to(args.device)
                    ...
                    print("\n------- Start Retrain --------")
                    retrain_model = get_model(self.arch_retrain, sample_shape, num_classes, config_kwargs, ks = ks, ds = ds, dropout = drop_rate)
                    retrain_train_loader, retrain_val_loader, retrain_test_loader, retrain_n_train, retrain_n_val, retrain_n_test, data_kwargs = get_data(args.dataset, accum * batch_size, self.arch_retrain, args.valid_split)
                ...
                test_time_start = default_timer()
                test_loss, test_score = evaluate(test_model, args.device, retrain_test_loader, loss, metric, retrain_n_test, self.decoder, self.transform, fsd_epoch=200 if args.dataset == 'FSD' else None)
                test_time_end = default_timer()
                test_scores.append(test_score)
