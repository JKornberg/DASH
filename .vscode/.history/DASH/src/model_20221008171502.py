...
#         # no of examples at each step/batch
        ...
        self.train_loader, self.val_loader, self.test_loader, self.n_train, self.n_val, self.n_test, self.data_kwargs = get_data(args.dataset, batch_size, arch, args.valid_split)
    ...
    def train(self, dataset, val_dataset=None, val_metadata=None, remaining_time_budget=None):
        ...
        ****************************************************************************
        ...
        for ep in range(epochs):
            train_loss = train_one_epoch(self.model, self.optimizer, self.scheduler, self.args.device, self.train_loader, loss, clip, 1, self.n_train_temp, self.decoder,
            ...
            if ep % validation_freq == 0 or ep == epochs - 1: 
                if args.baseline:
                    val_loss, val_score = evaluate(model, args.device, self.val_loader, loss, metric, n_val, decoder, transform, fsd_epoch=ep if args.dataset == 'FSD' else None)
