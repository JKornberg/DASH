        ...
        ****************************************************************************
        ...
        for ep in range(epochs):
            ...
            if not args.baseline and ((ep + 1) % retrain_freq == 0 or ep == epochs - 1):
                ...
                test_time_start = default_timer()
                test_loss, test_score = evaluate(test_model, args.device, retrain_test_loader, loss, sel.metric, self.retrain_n_test, self.decoder, self.transform, fsd_epoch=200 if args.dataset == 'FSD' else None)
                test_time_end = default_timer()
                test_scores.append(test_score)
