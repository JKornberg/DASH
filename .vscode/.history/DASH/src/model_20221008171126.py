        ...
        ****************************************************************************
        ...
        for ep in range(epochs):
            train_loss = train_one_epoch(self.model, self.optimizer, self.scheduler, self.args.device, train_loader, loss, clip, 1, self.n_train_temp, self.decoder,
                                         self.transform, self.lr_sched_iter, scale_grad=not self.args.baseline)
            ...
            if ep % validation_freq == 0 or ep == epochs - 1: 
                if args.baseline:
                    val_loss, val_score = evaluate(model, args.device, val_loader, loss, metric, n_val, decoder, transform, fsd_epoch=ep if self.args.dataset == 'FSD' else None)
